...
export default async function Article({
  ...
  const article = articles.find((article) => article.slug === params.articleId);
  if (!article) {
    // Option 1: Return null (will render nothing)
    // return null;
    // Option 2: Throw notFound to trigger Next.js 404 page
    // If you want a 404 page, uncomment:
    // import { notFound } from 'next/navigation';
    // notFound();
    // Option 3: Render a fallback UI
    return (
      <div className="text-center py-20">
        <h1 className="text-3xl font-bold mb-4">Article Not Found</h1>
        <p>The article you are looking for does not exist.</p>
        <Link href="/blog" className="text-blue-500 underline mt-4 inline-block">Back to Blog</Link>
      </div>
    );
  }
